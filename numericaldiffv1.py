...
class function():
    def __init__(self, a_v=15.8, a_s=17.3, a_c = 0.68, a_a = 23.3, a_p = 11):
        self.a_v = a_v
        self.a_s = a_s
        self.a_c = a_c
        self.a_a = a_a
...
def main():
   ...
   # for i in range(5):
    print("Initial value of derivative")
    print(derivative(BEfunction))
    print("Initial vector of parameters (starting conditions):")
    print(BEfunction.vector)
    onelist = [1,1,1,1,1]
    onevector=np.array(onelist)

    while magnitude(derivative(BEfunction)) > (1*(10**6)):

        BEfunction.vector-=(derivative(BEfunction)/(10**10))
    print("Local minimum coefficient values: ")
    print(BEfunction.vector)
